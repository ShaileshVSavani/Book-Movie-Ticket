...
//====
  ...
  //   // Retrieve all bookings from localStorage for the current user
  ...
  //   // Save bookings under the specific user
  //   localStorage.setItem(userBookingsKey, JSON.stringify(savedBookings));
  //   setErrorMessage("");
  //   navigate("/confirmation", {
  //     state: {
  //       movie,
...
//   useEffect(() => {
//     if (inputs.date && inputs.time) {
//       const allBookings = JSON.parse(localStorage.getItem(`booking_${userId}`)) || [];
//       const movieBooking = allBookings.filter(
//         (booking) => booking.movieId === id && booking.date === inputs.date && booking.time === inputs.time
//       );
